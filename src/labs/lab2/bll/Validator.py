...
class Validator:
    ...
    def is_equasion_incomplete_args(self, operator, num1, num2):
        if operator in self.unary_operations:
    ...
    def is_equasion_complete_args(self, operator, num1, num2, result):
        if operator in self.unary_operations:
    ...
    def is_equasion_incomplete(self, operation_object: Operation):
        try:
            operator, num1, num2, result = operation_object.to_components()
            is_equasion = self.is_equasion_incomplete_args(operator, num1, num2)
            return is_equasion
        except Exception as e:
            print("Can't use menthod from Operation. {e}")
            return False

    def is_equasion_complete(self, operation_object: Operation):
        try:
            operator, num1, num2, result = operation_object.to_components()
            return self.is_equasion_complete_args(operator, num1, num2, result)
        except Exception as e:
            print("Can't use menthod from Operation. {e}")
